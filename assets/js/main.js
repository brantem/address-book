...
inputAddress.addEventListener('keyup', e => e.keyCode == 13 ? insertAddress() : false) // insert address on enter
...
function insertAddress () {
  ...
  if (inputName.value !== '' || inputPhoneNumber.value !== '' || inputAddress.value !== '') {
    const addresses = getAddresses()
    const newAddress = { name, phoneNumber, address }

    window.localStorage.setItem('addresses', JSON.stringify({ addresses: [...addresses, newAddress]}))

    appendAddress(getAddresses().length, name, phoneNumber, address)
...
function appendAddress (number, name, phoneNumber, address) {
